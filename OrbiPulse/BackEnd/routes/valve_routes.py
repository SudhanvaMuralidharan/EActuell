...
from models.valve_model import ValveResponse, ValveCreate, ValveUpdate, ValveControl
...
@router.get(
    "",
    response_model=List[ValveResponse],
    summary="List valves, optionally filtered by zone",
)
async def list_valves(
    ...
    return await valve_service.list_valves_for_plot(db, zone)
...
@router.get(
    "/{valve_id}",
    response_model=ValveResponse,
    summary="Get a single valve by device_id",
)
async def get_valve(
    valve_id: str,
    db: AsyncSession = Depends(get_db),
...
async def control_valve(
    ...
    return await valve_service.control_valve(db, valve_id, body.action, body.position)
