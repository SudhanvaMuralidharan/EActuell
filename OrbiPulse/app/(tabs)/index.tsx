  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        <View>
          <Text style={styles.logo}>OrbiPulse</Text>
        ...
        <View style={styles.statsRow}>
          ...
          {faultCount > 0 && (
            <View style={[styles.statPill, { borderColor: Colors.red + '66' }]}>
              <View style={[styles.dot, { backgroundColor: Colors.red }]} />
              <Text style={[styles.statText, { color: Colors.red }]}>{faultCount} Fault</Text>
            </View>
          )}
        </View>
      </View>
      ...
      <View style={styles.fleetBar}>
        {[
          { label: 'Total',    val: VALVES.length,   color: Colors.textSecondary },
          { label: 'Open',     val: openCount,        color: Colors.accent },
          { label: 'Fault',    val: faultCount,       color: Colors.red },
          { label: 'Offline',  val: offlineCount,     color: Colors.offlineGray },
          { label: 'Gateways', val: GATEWAYS.length,  color: Colors.blue },
        ].map(({ label, val, color }) => (
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        ...
        {STATUS_FILTERS.map((s) => {
          ...
          const color = s === 'all' ? Colors.textPrimary : getStatusColor(s as ValveStatus);
          return (
...
const styles = StyleSheet.create({
  ...
  logo:   { fontSize: FontSize.lg, fontWeight: '800', color: Colors.accent, letterSpacing: 0.5 },
  subtitle: { fontSize: FontSize.xs, color: Colors.textSecondary, marginTop: 1 },
  statsRow: { flexDirection: 'row', gap: Spacing.xs },
  statPill: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: Colors.surface, borderRadius: Radius.full, paddingHorizontal: Spacing.sm, paddingVertical: 4, borderWidth: 1, borderColor: Colors.border },
  dot:      { width: 7, height: 7, borderRadius: 4 },
  statText: { fontSize: FontSize.xs, color: Colors.textPrimary, fontWeight: '600' },
  fleetBar: { flexDirection: 'row', justifyContent: 'space-around', backgroundColor: Colors.surface, borderTopWidth: 1, borderBottomWidth: 1, borderColor: Colors.border, paddingVertical: Spacing.sm, marginBottom: Spacing.xs },
  fleetItem: { alignItems: 'center' },
  fleetVal:  { fontSize: FontSize.xl, fontWeight: '800' },
  fleetLabel:{ fontSize: FontSize.xs, color: Colors.textMuted, marginTop: 1 },
  filterBar: { maxHeight: 44 },
  filterContent: { paddingHorizontal: Spacing.md, paddingVertical: Spacing.xs, gap: Spacing.xs },
  filterChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: Radius.full, borderWidth: 1, borderColor: Colors.border, backgroundColor: Colors.surface },
  filterText: { fontSize: FontSize.xs, color: Colors.textSecondary, fontWeight: '600' },
  filterCount:{ fontWeight: '400', opacity: 0.7 },
  sectionLabel: { fontSize: FontSize.xs, fontWeight: '700', color: Colors.textMuted, letterSpacing: 1, paddingHorizontal: Spacing.md, marginTop: Spacing.md, marginBottom: Spacing.sm },
  gwBlock: { marginHorizontal: Spacing.md, marginBottom: Spacing.sm, backgroundColor: Colors.surface, borderRadius: Radius.lg, borderWidth: 1, borderColor: Colors.border, overflow: 'hidden' },
  gwHeader: { flexDirection: 'row', alignItems: 'center', padding: Spacing.md, gap: Spacing.sm },
  gwIconWrap: { width: 36, height: 36, borderRadius: 18, backgroundColor: Colors.blue + '22', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: Colors.blue + '44' },
  gwInfo: { flex: 1 },
  gwName: { fontSize: FontSize.md, fontWeight: '700', color: Colors.textPrimary },
  gwMeta: { fontSize: FontSize.xs, color: Colors.textSecondary, marginTop: 1 },
  valveList: { borderTopWidth: 1, borderTopColor: Colors.border },
  noValves: { fontSize: FontSize.sm, color: Colors.textMuted, textAlign: 'center', padding: Spacing.md },
  valveRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: Spacing.sm, paddingRight: Spacing.sm, borderBottomWidth: 1, borderBottomColor: Colors.border, gap: Spacing.sm },
  valveBar: { width: 3, alignSelf: 'stretch', borderRadius: 2 },
  valveInfo: { flex: 1 },
  valveName: { fontSize: FontSize.sm, fontWeight: '600', color: Colors.textPrimary },
  valveMeta: { fontSize: FontSize.xs, color: Colors.textMuted, marginTop: 1 },
  valveRight: { alignItems: 'flex-end' },
  valvePos:  { fontSize: FontSize.sm, fontWeight: '800' },
  valveSeen: { fontSize: FontSize.xs, color: Colors.textMuted, marginTop: 1 },
  valveDetail: { backgroundColor: Colors.surfaceHigh, padding: Spacing.md, borderBottomWidth: 1, borderBottomColor: Colors.border },
  valveMetrics: { flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.xs, marginBottom: Spacing.sm },
  miniMetric: { minWidth: '30%', flex: 1, backgroundColor: Colors.surface, borderRadius: Radius.sm, padding: Spacing.xs, borderWidth: 1, borderColor: Colors.border },
  miniLabel: { fontSize: 10, color: Colors.textMuted },
  miniVal:   { fontSize: FontSize.sm, fontWeight: '700', color: Colors.textPrimary, marginTop: 1 },
  detailBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: Colors.accent, paddingVertical: Spacing.sm, borderRadius: Radius.md },
  detailBtnText: { fontSize: FontSize.sm, fontWeight: '700', color: Colors.bg },
  coordCard: { marginHorizontal: Spacing.md, backgroundColor: Colors.surface, borderRadius: Radius.lg, padding: Spacing.md, borderWidth: 1, borderColor: Colors.border },
  coordRow:  { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4 },
  coordLabel:{ fontSize: FontSize.xs, color: Colors.textSecondary },
  coordVal:  { fontSize: FontSize.xs, color: Colors.textPrimary, fontWeight: '600' },
});
